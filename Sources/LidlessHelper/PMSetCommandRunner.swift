...
enum ProcessCommandError: Error, Equatable {
    case invalidTimeout
    case timedOut
    case streamTooLarge
    case invalidUTF8
...
final class ProcessCommandRunner: CommandRunning {
    ...
    func run(executable: String, arguments: [String], timeout: TimeInterval) throws -> CommandResult {
        guard timeout > 0 else {
            throw ProcessCommandError.invalidTimeout
        }
        ...
        let readerQueue = DispatchQueue(label: "lv.ykv.lidless.helper.process-output", attributes: .concurrent)
        readers.enter()
        readerQueue.async {
            stdout.drain()
            readers.leave()
        ...
        guard let stdoutText = String(data: stdoutData, encoding: .utf8),
            ...
            throw ProcessCommandError.invalidUTF8
        }
...
private final class BoundedStreamCollector: @unchecked Sendable {
    ...
    func result() throws -> Data {
        ...
        if overflowed {
            throw ProcessCommandError.streamTooLarge
        }
        return data
    }
}
