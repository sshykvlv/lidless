...
public final class HelperEngine: @unchecked Sendable {
    ...
    public func arm(connectionID: UUID, sample: PowerSample, floor: BatteryFloor) throws -> UUID {
        try queue.sync {
            ...
            if case let .cutoff(reason) = SafetyPolicy.evaluate(sample: sample, floor: floor, now: wallClock.now) {
    ...
    public func disarm(connectionID: UUID, sessionID: UUID, reason: DisarmReason) throws {
        try queue.sync {
            if state == .inactive {
                if lastCompletedOwnership == OwnershipIdentity(connectionID: connectionID, sessionID: sessionID) {
                    return
                }
    ...
    public func connectionInvalidated(connectionID: UUID) throws {
        try queue.sync {
            guard state == .active,
                  ownership?.journal.connectionID == connectionID else {
                return
            }
    ...
    public func verifyInactiveForRestart() throws {
        try queue.sync {
            ...
            guard try journalStore.load() == nil,
                  try pmset.readSleepDisabled() == false else {
                throw HelperError.busy
            }
        }
    }
